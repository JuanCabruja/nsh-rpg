...
useEffect(() => {
  ...
  if (!habilidad || !daño || !costeChakra || !ejecutor || !receptor || !ronda) {
    ...
    console.log('Combatientes antes de actualizar:', combatientes);
    console.log('Ejecutor antes de actualizar:', ejecutorObj);
    console.log('Receptor antes de actualizar:', receptorObj);
  ...
  // Guardar en localStorage después de actualizar el estado
  const updatedCombatientes = combatientes.map((c, index) => {
    if (index === ejecutorIndex) return ejecutorObj;
    if (index === receptorIndex) return receptorObj;
    return c;
  });
  console.log('Combatientes después de actualizar:', updatedCombatientes);
  localStorage.setItem('combatientes', JSON.stringify(updatedCombatientes));
  ...
  console.log('Acciones después de agregar:', updatedAcciones);
